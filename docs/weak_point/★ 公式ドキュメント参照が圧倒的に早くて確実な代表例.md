以下のようにドキュメントを調整しました。内容に一貫性を持たせ、**`command`** とそれに関連する部分に焦点をあてました。
...
`/bin/sh` についての詳細は、公式ドキュメントの **[Attach Handler Lifecycle Event](https://kubernetes.io/docs/tasks/configure-pod-container/attach-handler-lifecycle-event/)** セクションを参照してください。
...
spec:
  ...
  - image: busybox  # 軽量なイメージを使用
    name: command-runner
    resources: {}
    command: ["/bin/sh", "-c", "echo Hello; echo Hello2"]  # コンテナが起動した際に実行されるコマンド
    # /bin/sh でシェルを起動し、`echo Hello` と `echo Hello2` の2つのコマンドを順番に実行します
